"""Train the challenging V2 farmer recommender with CUDA PyTorch.
...
DEFAULT_DATA = PROJECT / "outputs" / "farmer_synthetic_v2"
DEFAULT_OUTPUT = PROJECT / "outputs" / "farmer_model_v2_cuda"
SEED = 20260823
WINDOW_SIZE = 32
SENSOR_COUNT = 3
FRUIT_COUNT = 9
...
class FarmerRiskNetV2(nn.Module):
...
class ExportableFarmerRiskNetV2(nn.Module):
    def __init__(self, model: FarmerRiskNetV2) -> None:
        super().__init__()
        self.model = model
...
def run_epoch(model: FarmerRiskNetV2, loader: DataLoader, optimizer: torch.optim.Optimizer | None, scaler: torch.amp.GradScaler, class_weights: torch.Tensor) -> float:
...
@torch.no_grad()
def predict(model: FarmerRiskNetV2, loader: DataLoader) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def evaluate(model: FarmerRiskNetV2, loader: DataLoader, target_risks: np.ndarray, target_hours: np.ndarray, target_classes: np.ndarray) -> dict:
...
def evaluate_scenarios(model: FarmerRiskNetV2, features: np.ndarray, risks: np.ndarray, hours: np.ndarray, classes: np.ndarray, scenarios: np.ndarray, scenario_names: list[str], batch_size: int) -> dict:
    results = {}
...
def plot_report(history: list[dict], holdout: dict, ood: dict, output_dir: Path) -> None:
    ...
    axes[0, 0].set_title("V2 training on difficult synthetic data")
    ...
    figure.tight_layout()
    figure.savefig(output_dir / "farmer_v2_model_report.png", dpi=160, facecolor="white")
    plt.close(figure)
...
def main() -> None:
    ...
    parser.add_argument("--epochs", type=int, default=int(os.environ.get("RIPEN_FARMER_V2_EPOCHS", "45")))
    parser.add_argument("--batch-size", type=int, default=int(os.environ.get("RIPEN_FARMER_V2_BATCH_SIZE", "256")))
    parser.add_argument("--seed", type=int, default=SEED)
    args = parser.parse_args()
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is unavailable; refusing to train farmer V2 on CPU.")
    ...
    model = FarmerRiskNetV2(features.shape[1], FRUIT_COUNT).cuda()
    ...
    print(f"V2 data: train={train_mask.sum()} val={val_mask.sum()} holdout={holdout_mask.sum()} ood={ood_mask.sum()} features={features.shape[1]}", flush=True)
    ...
    if best_state is None:
        raise RuntimeError("No V2 model checkpoint was produced")
    ...
    torch.save({"state_dict": model.state_dict(), "history": history, "holdout": holdout, "ood": ood}, args.output / "farmer_v2_risk_cuda.pt")
    exported = ExportableFarmerRiskNetV2(model).eval().cuda()
    onnx_path = args.output / "farmer_v2_risk_cuda.onnx"
    ...
    metadata.update({
        ...
        "model_version": "farmer-synthetic-v2-experimental",
    ...
    (args.output / "farmer_v2_model_config.json").write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
    (args.output / "farmer_v2_model_history.json").write_text(json.dumps(history, indent=2) + "\n", encoding="utf-8")
